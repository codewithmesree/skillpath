...
export const InstructorSidebar = ({ activeItem }: SidebarProps) => {
  const menuItems = [
    { name: 'Dashboard', icon: LayoutDashboard, path: '/instructor' },
    { name: 'Create Course', icon: PlusCircle, path: '/instructor/create' },
    { name: 'Students', icon: Users, path: '/instructor/students' },
    { name: 'Earnings', icon: IndianRupee, path: '/instructor/earnings' },
    ...
    <aside className="w-[300px] bg-bg-offwhite border-r-4 border-deep-indigo min-h-screen p-8 hidden lg:flex flex-col gap-10 sticky top-[72px] h-[calc(100vh-72px)] z-20">
      <nav className="flex-1 space-y-4">
        {menuItems.map((item) => {
          ...
          const Icon = typeof item.icon === 'string' ? Settings : item.icon;
      ...
      <div className="bg-primary p-6 border-3 border-deep-indigo shadow-brutal -rotate-1 group hover:rotate-0 transition-transform duration-300">
        <div className="flex items-center gap-2 mb-2">
          <PlusCircle size={18} className="text-white" />
          <span className="font-heading font-black uppercase text-xs text-white">Top Creator</span>
        </div>
        <p className="font-body text-[11px] font-bold leading-tight mb-4 text-white/80">You're in the top 10% of creators this month! Keep it up.</p>
        <button className="w-full bg-white border-2 border-deep-indigo py-2 font-heading font-black text-[10px] uppercase hover:bg-deep-indigo hover:text-white transition-colors">View Rewards</button>
      </div>
